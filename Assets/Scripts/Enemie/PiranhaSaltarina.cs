...
public class PiranhaSaltarina : MonoBehaviour
{
    ...
    private float lastJumpTime = 0f; // Tiempo en que puede realizar el próximo salto
    ...
    IEnumerator PrepareAttack()
    {
        ...
        if (player != null)
        {
            ...
            isAttacking = true;
            lastJumpTime = Time.time; // Actualizar el tiempo del último salto
        }
    }
    ...
    void AttackPlayer()
    {
        if (!isJumping)
        {
            isJumping = true;  // La piraña empieza el salto
            rb.velocity = attackDirection * attackSpeed;
        }

        // Verificar si la piraña ha tocado el agua después del ataque
        if (isInWater)
        {
            isJumping = false;          // La piraña ha terminado el salto
            isAttacking = false;
            isResting = true;
